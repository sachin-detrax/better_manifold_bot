"""
Unit tests for enhanced OpenAI signal.
...
from better_manifold_bot.signals.OpenAI_signal import (
    OpenAISignal,
    ForecastRun,
    FORECAST_SCHEMA,
    create_user_prompt
...
class TestOpenAISignalDecomposition:
...
class TestOpenAISignalAggregation:
    ...
    def test_aggregate_consistent_runs(self):
        ...
        signal = OpenAISignal(variance_penalty_k=0.5)
        runs = [
...
class TestOpenAISignalDisagreementPenalty:
...
class TestOpenAISignalRetryLogic:
    """Test retry and fallback mechanisms."""

    @patch('better_manifold_bot.signals.OpenAI_signal.openai.OpenAI')
    def test_retry_on_json_error(self, mock_OPENAI):
        """Test retry logic when JSON parsing fails."""
        signal = OpenAISignal(n_runs=1)
        ...
        result = signal._call_OpenAI("test prompt")

        # Should retry and eventually succeed
        assert mock_client.messages.create.call_count == 2

    @patch('better_manifold_bot.signals.OpenAI_signal.openai.OpenAI')
    def test_fallback_on_all_failures(self, mock_OPENAI):
        """Test fallback when all retries fail."""
        signal = OpenAISignal(n_runs=2)
...
class TestOpenAISignalIntegration:
    """Integration tests."""

    @patch('better_manifold_bot.signals.OpenAI_signal.openai.OpenAI')
    def test_full_analysis_flow(self, mock_OPENAI):
        ...
        assert "OpenAI ensemble" in result.reasoning
...
class TestOpenAISignalEdgeCases:
    ...
    def test_confidence_clamping(self):
        ...
        signal = OpenAISignal(max_confidence=0.90)
        runs = [
    ...
    def test_probability_clamping(self):
        ...
        signal = OpenAISignal(variance_penalty_k=10.0)  # Very high penalty
        runs = [
